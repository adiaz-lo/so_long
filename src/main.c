...
t_map	*init_floor(t_mlx_player *mlx_player)
{
	mlx_player->map = malloc(sizeof(t_map));
	mlx_player->map->x = 0;
	mlx_player->map->y = 0;
	mlx_player->map->texture = mlx_load_png("floor.png");
	mlx_player->map->image_floor = mlx_texture_to_image(mlx_player->mlx, mlx_player->map->texture);
...
int32_t	main(void)
{
	...
	mlx_player->map = init_floor(mlx_player);
